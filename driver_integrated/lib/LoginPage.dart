...
class _LoginPageState extends State<LoginPage> {
  ...
  Widget build(BuildContext context) {
    Future<Map<String,dynamic>> makePostRequest(Map<String, String> requestBody) async {
        Map<String, dynamic> data = await MyApiService.driverLogIn(requestBody);
        dynamic authUser = data["auth_user"];
        String response_message = (data["auth_user"]["message"]);
        return data;
        ...
        //   String plateNumber = authUser["plate_number"];
        //   String jwtToken = authUser["jwt_token"];
        //   driver.initializeDriver(id, region, vehicleType, name, mobileNumber, plateNumber, jwtToken);
        //   MyApiService.updateToken(driver.id, firebaseService.fcmToken!, driver.jwtToken);
                        ...
                        Container(
                            ...
                            child: ElevatedButton(
                              onPressed: () async {
                                ...
                                Map<String,dynamic> returned_message = await makePostRequest(body);
                                ...
                                //initialise driver
                                if (returned_message["auth_user"]["message"] == "Login successfully.") {
                                  int id = returned_message["auth_user"]["id"];
                                  int region = returned_message["auth_user"]["region"];
                                  int vehicleType = returned_message["auth_user"]["vehicle_type"];
                                  String name = returned_message["auth_user"]["name"];
                                  int mobileNumber = returned_message["auth_user"]["mobile_number"];
                                  String plateNumber = returned_message["auth_user"]["plate_number"];
                                  String jwtToken = returned_message["auth_user"]["jwt_token"];
                                  driver.initializeDriver(
                                      id,
                                      region,
                                      vehicleType,
                                  ...
                                  MyApiService.updateToken(
                                      driver.id, firebaseService.fcmToken!,
                                      driver.jwtToken);
                                }
                                      ...
                                      content: Stack(
                                        children: [
                                          Container(
                                            ...
                                            child: Row(
                                              children: [
                                                if(returned_message["auth_user"]["message"] == "Login Failed")...[
                                                  if(username_value.text == "" && password_value.text == "")...[
                                                    const Text("Username and Password is empty.", style: TextStyle(fontSize: 18, color: Colors.red, fontWeight: FontWeight.bold),),
                                                  ]else if (username_value.text != "" && password_value.text == "")...[
                                                    const Text("Password is empty.", style: TextStyle(fontSize: 18, color: Colors.red, fontWeight: FontWeight.bold),),
                                                    ...
                                                    const Text("Invalid username or password.", style: TextStyle(fontSize: 18, color: Colors.red, fontWeight: FontWeight.bold),),
                                                  ]
                                                ]else...[
                                                  const Text("Login successfully.", style: TextStyle(fontSize: 18, color: Colors.green, fontWeight: FontWeight.bold),),
                                                ]
                                            ]),
                                          ),
                                        ],
